...
-- Select all rows and columns from the carbon_emission table
SELECT Year, Value
FROM `Carbon Dioxide Emission Estimates`
WHERE `CO2 emission estimates` = 'Albania' AND Series = 'Emissions per capita (metric tons of carbon dioxide)'
LIMIT 0, 1000;
...
CREATE TABLE emissions_new (
    Country VARCHAR(50) CHARACTER SET utf8mb4,
    Year INT,
    Series VARCHAR(100) CHARACTER SET utf8mb4,
    Value FLOAT
);
...
INSERT INTO emissions (Country, Year, Series, Value)
...
CREATE TEMPORARY TABLE value1975 AS
SELECT Country, Value AS old_value
FROM perCapital
WHERE Year = 1975;

CREATE TEMPORARY TABLE value2017 AS
SELECT Country, Value AS new_value
FROM perCapital
WHERE Year = 2017;

-- Now you can use the temporary tables value1975 and value2017 in your subsequent queries
